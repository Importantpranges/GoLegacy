:: GoLegacy: Offline Launcher
:: Author: KittyBooChanYes BongoNo
:: License: MIT
set GoLegacy_VER=1.2.3
title GoLegacy: Offline v%GoLegacy_VER% [Initializing...]
...
if not exist GoLegacy ( goto error_location )
if not exist server ( goto error_location )
goto noerror_location
:error_location
echo Doesn't seem like this script is in a GoLegacy: Offline folder.
pause && exit
:noerror_location
...
if not exist "utilities\checks\disclaimer.txt" (
	echo DISCLAIMER
  echo:
	echo GoLegacy: Offline is a project to preserve the original GoAnimate flash-based themes.
	...
	echo:
	echo Excluding Adobe Flash and GoAnimate Inc's assets, GoLegacy: Offline is free/libre software.
	...
	echo:
	echo By continuing to use GoLegacy: Offline, you acknowledge the nature of this project, and your right to use it.
	echo If you object to any of this, feel free to close GoLegacy: Offline now.
	echo You will be allowed to accept 20 seconds after this message has appeared.
	echo: 
	PING -n 21 127.0.0.1>nul
	echo If you still want to use GoLegacy: Offline, press Y. If you no longer want to, press N.
	:disclaimacceptretry
...
echo:
if !VERBOSEGoLegacy!==y ( echo Verbose mode activated. && echo:)
...
if !VERBOSEGoLegacy!==n (
	echo Checking for dependencies...
	echo:
)

title GoLegacy: Offline v!GoLegacy_VER! [Checking dependencies...]
...
:: Flash Player
if !VERBOSEGoLegacy!==y ( echo Checking for Flash installation... )
...
:: Node.js
if !VERBOSEGoLegacy!==y ( echo Checking for Node.js installation... )
...
:: http-server
if !VERBOSEGoLegacy!==y ( echo Checking for http-server installation... )
...
:: HTTPS cert
if !VERBOSEGoLegacy!==y ( echo Checking for HTTPS certificate... )
...
title GoLegacy: Offline v!GoLegacy_VER! [Installing dependencies...]
...
:: Skipped in Safe Mode, just in case anyone is running GoLegacy in safe mode... for some reason
:: and also because that was just there in the code i used for this and i was like "eh screw it why remove it"
if !ADMINREQUIRED!==y (
	if !VERBOSEGoLegacy!==y ( echo Checking for Administrator rights... && echo:)
	if /i not "!SAFE_MODE!"=="y" (
		fsutil dirty query !systemdrive! >NUL 2>&1
		if /i not !ERRORLEVEL!==0 (
			color cf
			if !VERBOSEGoLegacy!==n ( cls )
			echo:
			echo ERROR
			echo:
			if !FLASH_DETECTED!==n (
				if !NODEJS_DETECTED!==n (
					echo GoLegacy: Offline needs to install Flash and Node.js.
				) else (
					echo GoLegacy: Offline needs to install Flash.
				)
			) else (
				echo GoLegacy: Offline needs to install Node.js.
			)
			echo To do this, it must be started with Admin rights.
			echo:
			echo Close this window and re-open GoLegacy: Offline as an Admin.
			echo ^(right-click start_GoLegacy.bat and click "Run as Administrator"^)
			echo:
			if !DRYRUN!==y (
	...
	if !VERBOSEGoLegacy!==y ( echo Admin rights detected. && echo:)
)
:postadmincheck
...
if !FLASH_DETECTED!==n (
	...
	if !BROWSER_TYPE!==n (
		...
		echo GoLegacy: Offline will not be able to install Flash.
		...
		:chromium_chosen
		set BROWSER_TYPE=chrome && if !VERBOSEGoLegacy!==y ( echo Chromium-based browser picked. && echo:) && goto escape_browser_ask

		:firefox_chosen
		set BROWSER_TYPE=firefox && if !VERBOSEGoLegacy!==y ( echo Firefox-based browser picked. ) && goto escape_browser_ask
	)

	:escape_browser_ask
	echo To install Flash Player, GoLegacy: Offline must kill any currently running web browsers.
	echo Please make sure any work in your browser is saved before proceeding.
	echo GoLegacy: Offline will not continue installation until you press a key.
	echo:
	pause
	echo:
	...
	if !BROWSER_TYPE!==chrome (
		...
		if not exist "utilities\installers\flash_windows_chromium.msi" (
			...
			echo A normal copy of GoLegacy: Offline should come with one.
...
if !NODEJS_DETECTED!==n (
	...
	if !CPU_ARCHITECTURE!==64 (
		if !VERBOSEGoLegacy!==y ( echo 64-bit system detected, installing 64-bit Node.js. )
		...
		echo You can just keep clicking next until it finishes, and GoLegacy: Offline will continue once it closes.
		if !DRYRUN!==n ( msiexec /i "utilities\installers\node_windows_x64.msi" !INSTALL_FLAGS! )
		goto nodejs_installed
	)
	if !CPU_ARCHITECTURE!==32 (
		if !VERBOSEGoLegacy!==y ( echo 32-bit system detected, installing 32-bit Node.js. )
		if not exist "utilities\installers\node_windows_x32.msi" (
	...
	if !CPU_ARCHITECTURE!==what (
		...
		echo GoLegacy: Offline can't tell if you're on a 32-bit or 64-bit system.
		...
		echo:
		if "!cpuchoice!"=="1" if !DRYRUN!==n ( msiexec /i "utilities\installers\node_windows_x32.msi" !INSTALL_FLAGS! ) && if !VERBOSEGoLegacy!==y ( echo Attempting 32-bit Node.js installation. ) && goto nodejs_installed
	...
	if !NODEJS_DETECTED!==y (
		...
		if !errorlevel! == 0 (
			...
			if not exist "utilities\installers\http-server-master" (
				...
				echo A normal copy of GoLegacy: Offline should come with them.
				echo You should be able to find a copy on this website:
		...
		echo Seems either the install failed, or GoLegacy: Offline managed to skip it.
...
if !HTTPSCERT_DETECTED!==n (
	...
	if not exist "server\the.crt" (
		...
		echo GoLegacy: Offline requires this to run.
		echo You should get a "the.crt" file from someone else, or redownload GoLegacy: Offline.
	...
	if /i not "!SAFE_MODE!"=="y" (
		...
		if /i not !ERRORLEVEL!==0 (
			if !VERBOSEGoLegacy!==n ( cls )
			echo For GoLegacy: Offline to work, it needs an HTTPS certificate to be installed.
			echo If you have administrator privileges, you should reopen start_GoLegacy.bat as Admin.
			echo ^(do this by right-clicking start_GoLegacy.bat and click "Run as Administrator"^)
			echo:
...
:: Alert user to restart GoLegacy without running as Admin
if !ADMINREQUIRED!==y (
	color 20
	if !VERBOSEGoLegacy!==n ( cls )
	echo:
	...
	echo:
	echo GoLegacy: Offline no longer needs Admin rights,
	echo please restart normally by double-clicking.
	echo:
	echo If you saw this from running normally,
	echo GoLegacy: Offline should continue normally after a restart.
	echo:
	if !DRYRUN!==y (
...
color 0f
echo All dependencies now installed^^! Continuing with GoLegacy: Offline boot.
echo:
...
title GoLegacy: Offline v!GoLegacy_VER! [Loading...]
...
:: Open GoLegacy in preferred browser
if !INCLUDEDCHROMIUM!==n (
	if !CUSTOMBROWSER!==n (
		echo Opening GoLegacy: Offline in your default browser...
		if !DRYRUN!==n ( start http://localhost:4343 )
	) else (
		echo Opening GoLegacy: Offline in your set browser...
		echo If this does not work, you may have set the path wrong.
		if !DRYRUN!==n ( start !CUSTOMBROWSER! http://localhost:4343 )
	)
) else (
	echo Opening GoLegacy: Offline using included Chromium...
	pushd utilities\ungoogled-chromium
	if !APPCHROMIUM!==y (
...
echo GoLegacy: Offline has been started^^! The video list should now be open.

::::::::::::::::
:: Post-Start ::
::::::::::::::::

title GoLegacy: Offline v!GoLegacy_VER!
if !VERBOSEGoLegacy!==y ( goto GoLegacystarted )
:GoLegacystartedcls
cls
:GoLegacystarted

echo:
echo GoLegacy: Offline v!GoLegacy_VER! running
echo A project from VisualPlugin adapted by the W:O Team
echo:
if !VERBOSEGoLegacy!==n ( echo DON'T CLOSE THIS WINDOW^^! Use the quit option ^(0^) when you're done. )
if !VERBOSEGoLegacy!==y ( echo Verbose mode is on, see the two extra CMD windows for extra output. )
...
echo Enter 0 to close GoLegacy: Offline
set /a _rand=(!RANDOM!*67/32768)+1
if !_rand!==25 echo Enter things you think'll show a secret if you're feeling adventurous
:GoLegacyidle
echo:
set /p CHOICE=Choice:
if "!choice!"=="0" goto exitGoLegacyconfirm
set FUCKOFF=n
if "!choice!"=="1" goto reopen_webpage
if "!choice!"=="2" goto start_importer
if "!choice!"=="3" goto open_server
if "!choice!"=="?" goto open_faq
if /i "!choice!"=="clr" goto GoLegacystartedcls
if /i "!choice!"=="cls" goto GoLegacystartedcls
if /i "!choice!"=="clear" goto GoLegacystartedcls
:: funni options
if "!choice!"=="43" echo OH MY GOD. FOURTY THREE CHARS. NOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO & goto GoLegacyidle
if /i "!choice!"=="benson" echo watch benson on youtube & goto GoLegacyidle
if /i "!choice!"=="ford" echo what up son & goto GoLegacyidle
if /i "!choice!"=="no" echo stahp & goto GoLegacyidle
if /i "!choice!"=="yes" echo Alright. & goto GoLegacyidle
if /i "!choice!"=="fuck off" goto youfuckoff
if /i "!choice!"=="fuck you" echo No, fuck you. & goto GoLegacyidle
if /i "!choice!"=="sex" echo that's fake & goto GoLegacyidle
...
if /i "!choice!"=="narutofan420" echo i am narutofan420 i am a naruto fan i watch naruto i watched all 3 series and still watch it & goto GoLegacyidle
if /i "!choice!"=="die" echo die please & goto GoLegacyidle
if /i "!choice!"=="aaron doan" echo YOU^^!^^!^^! Noo GoLegacy Is Patched Forever^^!^^!^^! Cries And Hits You So Many Times & goto GoLegacyidle
:: dev options
if /i "!choice!"=="amnesia" goto wipe_save
if /i "!choice!"=="restart" goto restart
if /i "!choice!"=="folder" goto open_files
echo Time to choose. && goto GoLegacyidle
...
:open_files
pushd ..
echo Opening the GoLegacy-offline folder...
start explorer.exe GoLegacy-offline
popd
goto GoLegacyidle
...
if !errorlevel! equ 1 goto GoLegacyidle
...
if /i "!patchchoice!"=="y" echo too bad B^) & goto GoLegacyidle
if /i "!patchchoice!"=="n" echo good & goto GoLegacyidle
...
if !_rand!==15 echo GoLegacy inline
goto GoLegacyidle
...
:exitGoLegacyconfirm
echo:
echo Are you sure you want to quit GoLegacy: Offline?
...
if /i "!exitchoice!"=="n" goto GoLegacystartedcls
if /i "!exitchoice!"=="no" goto GoLegacystartedcls
if /i "!exitchoice!"=="with style" goto exitwithstyle
echo You must answer Yes or No. && goto exitGoLegacyretry

:point_extraction

title GoLegacy: Offline v!GoLegacy_VER! [Shutting down...]
...
echo GoLegacy: Offline has been shut down.
if !FUCKOFF!==y ( echo You're a good listener. )
echo This window will now close.
if !INCLUDEDCHROMIUM!==y (
	echo You can close the web browser now.
)
echo Open start_GoLegacy.bat again to start W:O again.
if !DRYRUN!==y ( echo Go wet your run next time. ) 
pause & exit

:exitwithstyle
title GoLegacy: Offline v!GoLegacy_VER! [Shutting down... WITH STYLE]
echo SHUTTING DOWN THE GoLegacy OFFLINE
PING -n 3 127.0.0.1>nul
color 9b
...
PING -n 3 127.0.0.1>nul
echo TIME TO ELIMINATE GoLegacy OFFLINE
PING -n 3 127.0.0.1>nul
echo BOBOOBOBMWBOMBOM SOUND EFFECTSSSSS
PING -n 3 127.0.0.1>nul
echo GoLegacy OFFLINE ALSO ANNIHILA
...
echo :: GoLegacy: Offline Config>> utilities\config.bat
...
echo set VERBOSEGoLegacy=n>> utilities\config.bat
echo:>> utilities\config.bat
